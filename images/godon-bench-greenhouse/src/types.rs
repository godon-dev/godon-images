...
pub struct MetricsResponse {
    ...
    /// Growth rate per unit of water consumed. **Objective to maximize.**
    /// Penalizes wasteful irrigation -- higher is better.
    pub water_efficiency: f64,
    /// Accumulated crop quality (0.0-1.0). Degrades with stress events,
    /// slowly recovers. Affected by damage_factor. **Objective to maximize.**
    ...
    /// Current outside temperature (°C). Pure weather, not affected by coupling.
    pub outside_temp: f64,
    /// Current outside CO2 (ppm). Baseline 420ppm.
    pub outside_co2: f64,
    /// Current outside humidity (ratio). Baseline weather model.
    ...
    /// Coupling thermal delta (°C/step). Shared thermal mass effect on zone temp.
    /// Applied directly to zone temperature each timestep.
    pub coupling_delta_temp: f64,
    /// Coupling CO2 permeation delta (ppm/step). Fick's law diffusion through shared wall.
    /// Applied directly to zone CO2 each timestep.
    pub coupling_delta_co2: f64,
    /// Coupling light/power sag delta (fraction). Kept for backward compat (now 0.0).
    pub coupling_delta_light: f64,
    /// Coupling water pressure delta (fraction). Shared water source pressure reduction.
    /// Reduces effective irrigation rate.
    pub coupling_delta_water: f64,
    /// Coupling humidity exchange delta (ratio/step). Semi-permeable wall moisture transfer.
    /// Applied directly to zone humidity each timestep.
...
pub struct StatusResponse {
    ...
    pub outside_temp: f64,
    /// Outside CO2 (ppm). Baseline value from weather model.
    pub outside_co2: f64,
    /// Outside humidity (ratio). Baseline value from weather model.
    pub outside_humidity: f64,
    pub solar_radiation: f64,
    pub tick: u64,
    pub trial_energy_kwh: f64,
